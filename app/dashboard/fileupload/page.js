...
import { 
  ...
  FileText, Image as ImageIcon, Music, Printer, Save, GripVertical
} from 'lucide-react';
...
export default function AdminPortal() {
  ...
  // --- NEW: State to track reorder saving ---
  ...
  const handleDragEnd = async (result) => {
    // Drop outside the list or currently already saving
    ...
    try {
      ...
      fetchCards(); // Revert to database state if failed
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.id) {
        payload.sortOrder = cards.length; 
    }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-4">
            ...
            {/* NEW: Display saving indicator next to header when reordering */}
            {isReordering && (
              <div className="flex items-center gap-2 text-sm font-medium text-orange-600 bg-orange-100 px-3 py-1 rounded-full animate-pulse">
                   ...
                   {(provided) => (
                     ...
                     >
                       {cards.map((card, index) => (
                         <Draggable key={card.id} draggableId={card.id} index={index} isDragDisabled={isReordering}>
                           {(provided, snapshot) => (
                             <div 
                               ref={provided.innerRef}
                               {...provided.draggableProps}
                               className={`bg-white p-5 rounded-xl shadow-sm border ${snapshot.isDragging ? 'border-orange-500 shadow-lg scale-[1.02]' : 'border-gray-100'} flex flex-col md:flex-row items-center justify-between hover:shadow-md transition-all z-50`}
                             >
                               <div className="flex items-center gap-4 w-full md:w-auto">
                                 {/* Drag Handle */}
                                 <div 
                                   {...provided.dragHandleProps}
                                   className={`p-2 ${isReordering ? 'cursor-not-allowed opacity-50' : 'cursor-grab hover:text-orange-600'} text-gray-400`}
                                 >
                               ...
                               <div className="flex gap-2 mt-4 md:mt-0 w-full md:w-auto ml-10 md:ml-0">
                                 <button onClick={() => handleEdit(card)} disabled={isReordering} className="flex-1 px-4 py-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 font-medium text-sm disabled:opacity-50">Edit</button>
                                 <button onClick={() => handleDelete(card.id)} disabled={isReordering} className="flex-1 px-4 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 font-medium text-sm disabled:opacity-50">Delete</button>
                               </div>
                             </div>
                           )}
                         </Draggable>
